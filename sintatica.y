...
VALOR		: TK_NUM
			...
			/*	if($1.ehDinamica)
					...
					$$.traducao = $1.traducao + '\t' + $$.label + " = sizeof(" + recuperarNomeTraducao($1.label, $1.escopoDeAcesso) + ");\n";
...
ARRAY	: TIPO '[' DIMENSOES_INDICES ']' //Criação de array
		...
		*/
		...
		ACESSO_ARRAY DIMENSOES_INDICES ']' //ACESSO_ARRAY DIMENSOES_INDICES ']'
		{
			...
			{
				string p[1] = {$1.label};
				yyerror(montarMensagemDeErro(MSG_ERRO_VARIAVEL_PRECISA_CONTER_UM_ARRAY, p, 1));
			}
...
DIMENSOES_INDICES	: DIMENSOES_INDICES ',' E
					{
						...
						else if($3.estruturaDoConteudo == constante_estrutura_variavel)
						{
							...
							adicionarTamanhoDimensoesArray(recuperarNomeTraducao($3.label, $3.escopoDeAcesso));
							adicionarTamanhoDimensoesArray(recuperarNomeTraducao($3.label, $3.escopoDeAcesso),&valoresReaisDim,true);
							count_dim++;
						}
					...
					E
					{
						...
						else if($1.estruturaDoConteudo == constante_estrutura_variavel) //É variável, é do tipo inteiro e existe.
						{
							...
							adicionarTamanhoDimensoesArray(recuperarNomeTraducao($1.label, $1.escopoDeAcesso));
							adicionarTamanhoDimensoesArray(recuperarNomeTraducao($1.label, $1.escopoDeAcesso),&valoresReaisDim,true);
							count_dim++;
...
ID			: TK_ID
			...
			'(' TK_BACKSCOPE TK_NUM ')' TK_ID
			{
				...
				if(variavelJaDeclarada($5.label, true, escopo)){
					...
					$$.nomeIdOriginal = $5.label; //Subir nome original.
...
COMANDO_SWITCH	: EMPILHAR_TAG_SWITCH TK_SWITCH '(' E ')' '{' CASES DEFAULT'}'
				{
					...
					if($4.tipo == $7.tipo) {
						...
						$$.traducao = substituirVariaveisCase($$.traducao, recuperarNomeTraducao($4.label, $4.escopoDeAcesso));
						removerTopoTagFim();
		...
		else if(tipo == constante_tipo_array)
		{
			...
			if(acessoArray)
			{
				dolarDolar.traducaoDeclaracaoDeVariaveis =  dolar4.traducaoDeclaracaoDeVariaveis +
															...
															"; //" + labelPrefix + "\n";
				dolarDolar.traducao = dolar4.traducao + "\t" + label + " = " + recuperarNomeTraducao(dolar4.nomeIdOriginal, dolar4.escopoDeAcesso) + "[" + dolar4.labelIndice + "];\n";
...
ATRIBUTOS tratarAtribuicaoVariavel(ATRIBUTOS dolar1, ATRIBUTOS dolar3, bool ehDinamica)
{
	...
	string labelRecuperada = recuperarNomeTraducao(dolar1.label, dolar1.escopoDeAcesso);
			...
			else if(dolar3.tipo == constante_tipo_array) //CASO ARRAY
			{
				...
				if(nuncaAtribuida)
				{
					...
					else //Atribuição de valor dentro do array. // Caso: a = livia[x,y] (a já existe, mas está sem valor.)
					{
						...
						dolarDolar.traducao = dolar3.traducao + "\t" + labelRecuperada + " = " + recuperarNomeTraducao(dolar3.label, dolar3.escopoDeAcesso) +
												"[" + dolar3.labelIndice + "];\n";
					...
					else //Atribuição de valor dentro do array.
					{
						...
						*/
						...
						else if(dolar1.acessoArray && dolar3.acessoArray) //livia[x,y] = braida[u,v];
						{
							...
							dolarDolar.traducao = dolar1.traducao + dolar3.traducao + recuperarNomeTraducao(dolar1.label, dolar1.escopoDeAcesso) + "[" +
													dolar1.labelIndice + "] = " + recuperarNomeTraducao(dolar3.label, dolar3.escopoDeAcesso) + "[" +
													dolar3.labelIndice + "];\n";
							//dolar1.tipo = tipoDolar1;
						}
