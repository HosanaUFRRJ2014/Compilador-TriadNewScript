...
{
       0,    68,    68,    75,    80,    88,    96,    99,   101,   103,
     105,   107,   109,   115,   117,   121,   131,   138,   147,   153,
     159,   164,   173,   181,   185,   205,   207,   209,   211,   213,
     262,   267,   270,   285,   333,   397,   412,   436,   451,   459,
     477,   486,   503,   505,   507,   525,   531,   537,   540,   546,
     552,   558,   566,   578,   580,   597,   600,   621,   630,   639,
     649,   667,   684,   705,   715,   720,   723,   726,   732
};
#endif
...
#line 186 "sintatica.y" /* yacc.c:1646  */
	...
	//			cout << "\n//Entrou em ID TIPO\n";
				...
				if((yyvsp[0]).tipo == constante_tipo_booleano)
					adicionarDefinicaoDeTipo((yyvsp[-2]).label, (yyvsp[0]).label,(yyvsp[-2]).tamanho + (yyvsp[0]).tamanho);
				
				else
					adicionarDefinicaoDeTipo((yyvsp[-2]).label, (yyvsp[0]).tipo,(yyvsp[-2]).tamanho + (yyvsp[0]).tamanho);
    ...
    {	
				...
				else
				{
					...
					//	$2.label = prefixo_variavel_usuario + $2.label;
						(yyval).traducaoDeclaracaoDeVariaveis = (yyvsp[-1]).traducaoDeclaracaoDeVariaveis + "\t" + tipo + " " + label + "[" + to_string(global_tamanhoStringConcatenada) + "];\n";
						(yyval).traducao = (yyvsp[-1]).traducao + montarCopiarString(label, (yyvsp[-1]).label) + ";\n";
...
#line 334 "sintatica.y" /* yacc.c:1646  */
    {
				...
				if((yyvsp[-3]).label != (yyvsp[-1]).label)
				{
					...
					if(metaData.tipo == "")
					{
						...
						if((yyvsp[-3]).escopoDeAcesso >= 0){
							adicionarDefinicaoDeTipo((yyvsp[-3]).label, tipo, (yyvsp[-3]).tamanho + (yyvsp[-1]).tamanho, (yyvsp[-3]).escopoDeAcesso);
							atualizarNoMapa(metaData, (yyvsp[-3]).escopoDeAcesso);
						}
						else{
							adicionarDefinicaoDeTipo((yyvsp[-3]).label, tipo,(yyvsp[-3]).tamanho + (yyvsp[-1]).tamanho);
							atualizarNoMapa(metaData);
						}
...
#line 739 "sintatica.y" /* yacc.c:1906  */
